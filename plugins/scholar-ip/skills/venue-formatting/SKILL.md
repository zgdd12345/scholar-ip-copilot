...
phase: paper
description: >
  Load when /scholar:paper-venue runs, or when /scholar:paper-draft is invoked with a
...
triggers:
  - "/scholar:paper-venue"
  - "/scholar:paper-draft with non-default style"
...
Load when running `/scholar:paper-venue` (the dedicated conversion command) or `/scholar:paper-draft` with a non-default `style` argument. The manuscript at `manuscript/` is **always** arXiv-neutral; this skill produces a parallel `submissions/<venue>/` tree without modifying the source.
...
- `skills/venue-formatting/venues/<id>.yaml` (the venue spec — required)
...
1. **Resolve the spec.** Read `skills/venue-formatting/venues/<venue>.yaml`. If the file is missing, abort with an error pointing to the supported list in `plugin.yaml::paper_defaults.supported_venues`.
...
9. **Write `MANIFEST.md`.**
   ...
   - venue spec: skills/venue-formatting/venues/<venue>.yaml
...
All specs live at `skills/venue-formatting/venues/<id>.yaml`. The MVP set:
