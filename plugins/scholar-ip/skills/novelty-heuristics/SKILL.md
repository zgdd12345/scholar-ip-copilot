...
Runs as a sub-pass of `/scholar:patent-review` (the existing 5-role panel) to give the `novelty-critic` agent **structured data instead of free-text prose**. The findings file is **advisory** — it scores per-element prior-art overlap so an attorney can triage the strongest hits, but it does **not** produce a patentability verdict, and the per-claim `verdict_hint` is an attorney-triage label, never a legal conclusion.

Load it from:

- `/scholar:patent-review` — automatically, before the `novelty-critic` subagent assembles its report section.
