...
- `/scholar:paper-lit` — every new BibTeX entry passes through this skill before being committed.
- `/scholar:paper-check` — missing-cite and unused-entry passes.
- Any one-off "is this key consistent?" question from the user.

## Shared cache + run_id convention

This skill is read+edit only; it does not retrieve from the web and writes no cache files. It **does** participate in the shared run_id convention: when called from `/scholar:paper-check` or `/scholar:paper-lit`, every audit row this skill emits to chat carries the caller's `run_id` (from the caller's `plan.yaml`) so the report can be cross-referenced.
...
- (`/scholar:paper-check`) the corresponding sections of `.evidraft/manuscript/paper_check_report.md`
...
Unused entries are **flagged, never auto-deleted**. The author may keep an entry for an upcoming draft section. The `/scholar:paper-check` report includes the list under "Unused keys".
