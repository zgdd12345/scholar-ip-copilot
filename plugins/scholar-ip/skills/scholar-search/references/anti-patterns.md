...
- Skipping the cache check — every fetch must consult `.evidraft/literature/.cache/<provider>/` (see SKILL.md §"On-disk layout: disposable cache vs. durable snapshots", Tier 1).
...
- Deleting OR overwriting a Tier 2 `snapshots/<sha1>.{md,json}` file referenced by any row in `evidence.jsonl` — that is the durable backing for an immutable historical claim. To refresh a snapshot whose upstream page has changed, fetch a NEW `<sha1>` and emit a new evidence row with `supersedes` pointing at the prior row. See SKILL.md §Tier 2 refresh recipe.
