...
provides:
  ...
  - "prompt-rendering recipe (persona body + target file + optional schema)"
...
allowed_tools:
  ...
  - "Bash:claude*"
  - "Bash:opencode*"
hooks: [external-write-zone, sensitive-file-guard]
references:
...
<EXTREMELY-IMPORTANT>
When `/scholar:xreview` runs, follow this skill exactly. Never invent
CLI flags. Never put a key or user content in argv. Never let the
external agent write outside `.evidraft/reviews/`.
</EXTREMELY-IMPORTANT>

This skill is the implementation contract for `/scholar:xreview` and for
any future `--fanout <agent>` mode on `/scholar:paper-review` and
`/scholar:patent-review`.

## CLI invocation matrix

The three invocations below are **verbatim** — the command file
`commands/xreview.md` and any adapter MUST use them exactly.
...
1. **Header** (fixed):
   ...
   ```
2. **Persona body** — the markdown body of
   `plugins/scholar-ip/agents/<persona>.md` **after** stripping its
   YAML frontmatter. Personas are reused as-is; this skill never
   invents new reviewer voices.
...
- [ ] **Read-only flags applied where supported.** Codex →
      ...
      worktree / copy.
- [ ] **Write zone pinned.** `external-write-zone` hook with
      `failure_mode: block` rejects any file appearing outside
      `.evidraft/reviews/` after the invocation.
- [ ] **`sensitive-file-guard` pre-check.** Reject if `target` matches
      `.env*`, `secrets/`, `**/*.pem`, `**/*.key`,
...
- [ ] **Hard timeout.** 600 seconds per invocation. On timeout, kill
      ...
      message.
- [ ] **No nested invocation.** `/scholar:xreview` may not call
      `/scholar:xreview` again from inside the external agent's
      response.
...
| `external-write-zone` violation | Block; delete any file the external agent wrote outside `.evidraft/reviews/`; do not retry. |
...
- Invent a new persona. Always render an existing `agents/<persona>.md`.
...
- Skip the `sensitive-file-guard` pre-check on `target`.
