...
allowed_tools: [Read, Glob, Grep, Write, Edit, "Bash:grep*"]
...
references:
  ...
  - doc: ../skills/novelty-heuristics/SKILL.md
...
Before the panel runs, refresh the structured audits (idempotent — they read whatever is on disk; if `/scholar:patent-claims` already produced fresh artefacts in this run_id, they no-op):

1. **Claim parsing.** Drive `skills/claim-parser/SKILL.md`. Reads `claims.md`; emits `claims_parsed.json` (canonical) + `claim_parse-<ts>.log`. Surfaces structural warnings (`MISSING_TERMINUS`, `UNNUMBERED_ELEMENT`, `FORWARD_DEPENDENCY`, `EMPTY_CLAIM` are `fail`-severity; `ANTECEDENT_MISSING`, `MULTIPLE_DEPENDENCY`, `TRANSITION_UNKNOWN`, `DEPENDENT_NO_NARROW` are `warn`).
2. **Claim chart.** Drive `skills/claim-chart-builder/SKILL.md`. Emits `claim_chart.md` + `claim_chart-<ts>.json`. Populates spec/code support, per-element prior-art overlap, risk roll-up, suggested revisions.
3. **Novelty heuristics.** Drive `skills/novelty-heuristics/SKILL.md`. Reads `claims_parsed.json` + `prior_art_map.md` (+ optional `claim_chart-<ts>.json` to reuse rows). Emits `novelty_audit-<ts>.findings.json` + paired `.log`. Produces `verdict_hint ∈ {novel, narrow, redraft, withdraw}` **advisory only** per claim.
