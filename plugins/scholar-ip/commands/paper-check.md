...
allowed_tools: [Read, Glob, Grep, Write, Edit, "Bash:latexmk*", "Bash:bibtex-tidy*", "Bash:grep*", "Bash:awk*"]
hooks: [citation-guard, evidence-consistency, latex-compile]
subagents: [evidence-auditor, methodology-reviewer, latex-editor, consistency-checker]
references:
  - doc: ../skills/evidence-check/SKILL.md
  - doc: ../skills/latex-writing/SKILL.md
  - doc: ../skills/latex-build/SKILL.md
  - doc: ../skills/latex-style-audit/SKILL.md
  - doc: ../skills/bib-manager/SKILL.md
  - doc: ../skills/bib-audit/SKILL.md
  - doc: ../skills/xref-audit/SKILL.md
---
...
3. **Style audit.** Drive via `skills/latex-style-audit/SKILL.md` (runs only when Step 2 is not `FAIL`):
   ...
   - Echo any `STRONG_CLAIM_VERB_NO_CITE` rows from the `citation-guard` hook output rather than re-running the verb scan.
...
4. **Bib quality audit.** Drive via `skills/bib-audit/SKILL.md` (runs after Step 1's `bib-manager` mechanics):
...
5. **Cross-reference audit.** Drive via `skills/xref-audit/SKILL.md` (runs after Step 2's `latex-build` compile):
...
9. **Consistency audit (semantic).** Delegate to `agents/consistency-checker.md`:
