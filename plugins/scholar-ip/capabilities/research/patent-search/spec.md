...
description: >
  ...
  prior-art map, or running `workflow:patent.scout` or
  `workflow:patent.prior-art`. Advisory only — not a freedom-to-operate
  analysis.
triggers:
...
references:
  - doc: capability:patent-disclosure
  - doc: capability:patent-claims
  - doc: capability:evidence-check
  - doc: workflow:patent.scout
  - doc: workflow:patent.prior-art
...
- `workflow:patent.scout` — surface candidate inventions with adjacent prior art.
- `workflow:patent.prior-art` — build `prior_art_map.md` and feed `claim_chart.md`.
