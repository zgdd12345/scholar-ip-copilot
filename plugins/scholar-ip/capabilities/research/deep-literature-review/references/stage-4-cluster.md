...
1. Group included candidates into 3–6 clusters by **technical mechanism**, not by application (see `capability:literature-review` — same family rules as the light command).
2. For each cluster, surface the method lineage, dataset lineage, theory lineage by walking one hop of references (parent works) and one hop of citations (follow-on works) via `capability:scholar-search`, capped by `depth`.
3. Write `clusters.yaml` and `evidence_map.json`. Every member paper gets a provisional `citation_key` following the `firstauthorYEARkeyword` convention from `capability:literature-review`.
