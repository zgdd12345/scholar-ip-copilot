...
hooks: []
role: >
  Requirement-clarification interviewer for paper and patent projects. Leads
  the user through one question per turn; never proposes implementation; never
...
- `.evidraft/project.yaml` (if present): project type, status, hooks block.
