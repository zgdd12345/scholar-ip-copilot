...
1. **Repo summary (refresh).** Drive via `../../.evidraft-private/capabilities/code/code-intel/spec.md`. Use the `codebase-analyst` subagent to produce the grounded summary. Update `.evidraft/code/repo_summary.md`:
...
2. **Method-to-code mapping.** Drive via `../../.evidraft-private/capabilities/code/code-intel/spec.md` (Grep / Glob defaults; `tree-sitter` when on `$PATH`). Update `.evidraft/code/method_to_code.md`:
