...
Build a literature foundation for the paper. Online retrieval uses the host-native `WebSearch` + `WebFetch` tools driven by the `scholar-search` skill (`../../.evidraft-private/capabilities/research/scholar-search/spec.md`); **if the host has no network, operate only on what the user already provided** (PDFs in `references/`, BibTeX in `references.bib`, notes in `.evidraft/literature/`).
...
1. **Inventory existing material.**
   ...
   - Ensure the first non-blank lines of `matrix.md` are the fixed banner below (insert if missing, do not duplicate):
     ...
     ```
2. **Online retrieval (optional).** If the user supplies a topic, load the `scholar-search` skill and request up to N (default 20) candidate papers per the URL templates / rate-limit policy in `../../.evidraft-private/capabilities/research/scholar-search/spec.md` (arXiv, Semantic Scholar, OpenAlex). For each candidate produce a structured stub (title, authors, year, venue, abstract, why-relevant). All bib edits go through `../../.evidraft-private/capabilities/evidence/bib-manager/spec.md` for dedup + key normalisation.
   - **Non-paper sources** (blog posts, vendor docs, engineering reports, tutorials, specs): use the `scholar-search` `webfetch` variant (`../../.evidraft-private/capabilities/research/scholar-search/spec.md` §Tier 2). It passes the exact fetched body to `evidraft snapshot store`, which writes `.evidraft/literature/snapshots/<sha256(raw_body)>.md`. The `snapshots/` tree is durable evidence backing and is never auto-deleted. Record the returned path for step 3.
...
- `references.bib` passes the validation chain described in `../../.evidraft-private/capabilities/evidence/bib-manager/spec.md §5` and `../../.evidraft-private/capabilities/evidence/bib-audit/spec.md §5.1` (prefer `bibtex-tidy`; fall back to the hand-rolled parser the same skills use). Chat output reports `validator_used: bibtex-tidy | hand-roll`.
