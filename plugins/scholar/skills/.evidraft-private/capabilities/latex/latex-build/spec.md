...
description: >
  ...
  structured error taxonomy consumed by paper checks and venue conversion (`MISSING_CITE`,
  ...
  `workflow:paper.check` or `workflow:paper.venue`.
triggers:
...
- `workflow:paper.check` — compile + classify errors for the audit report.
- `workflow:paper.venue` — compile sanity-check after the preamble rewrite.
- Paper check and venue actions invoke this same recipe explicitly after relevant `.tex` writes.
...
For `workflow:paper.venue`, the working directory is `submissions/<venue>/`; for everything else, `<project root>/manuscript/`.
...
Walk the log line-by-line. Classify each error / warning into one of six categories. For each: a **regex pattern**, a **file:line extraction rule**, and a **fix-suggestion template**.
...
`compile: "PASS"` iff `latexmk` exit code is 0 **and** no `! ` lines remain in the log. Any unresolved `\ref` / `\cite` warning downgrades to `FAIL` for `workflow:paper.venue` (the venue copy must build cleanly to submit) but only to `WARN` for `workflow:paper.check` (the working manuscript can still have open ends).
