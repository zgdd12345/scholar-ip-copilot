...
references:
  ...
  - doc: policy:evidence-integrity
  - doc: policy:evidence-integrity
  - doc: role-mode:latex-editor
...
Runs as a sub-pass of `workflow:paper.check` **after** `latex-build` reports a clean (or merely warning-level) compile. The skill is observational: it never edits `.tex`.
...
|---|---|---|
| What to look for | [rule-taxonomy.md](references/rule-taxonomy.md) | 28 rules in 7 groups (captions, cross-references, math, tables&figures, bib proximity, microtypography, common misuses); severity policy; per-rule patterns + examples + suggested fixes + known false-positive notes; policy:evidence-integrity echo recipe |
...
| What to avoid | [anti-patterns.md](references/anti-patterns.md) | the 8 anti-patterns (auto-rewrite, comment false-positives, re-implementing policy:evidence-integrity, rule inflation, severity creep, skipping empty findings.json, running on FAIL compile, verbatim hacks) |
...
- [ ] When `compile == "FAIL"` from the preceding `latex-build`, the skill skips the run (with one chat line explaining why) unless the caller is `workflow:paper.venue`.
