...
phase: patent
description: >
  Parses `.evidraft/patent/claims.md` (the output of `workflow:patent.claims`)
...
triggers:
  - "command:workflow:patent.claims"
...
references:
  ...
  - doc: workflow:patent.claims
  - doc: workflow:patent.review
...
- as a sub-pass of `workflow:patent.claims` immediately after `claim-drafter` writes the file, so the rest of the pipeline can read JSON instead of prose;
- as a sub-pass of `workflow:patent.review`, so the `claim drafter` reviewer role and the `novelty critic` role share the same structural view;
...
Skip the run if `claims.md` does not exist; emit a single chat line and exit cleanly (this is the normal case before `workflow:patent.claims` has ever produced output).
