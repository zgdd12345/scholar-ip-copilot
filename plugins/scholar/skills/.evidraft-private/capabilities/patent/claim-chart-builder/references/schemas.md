...
- `claim_chart.md` — human-readable, **replaces the previous flat-template version** (the template at `../../../../templates/patent-project/.evidraft/patent/claim_chart.md` is a stub; this capability writes the live, populated file).
...
Plus a trailing H2 `## Summary` block that renders the JSON `summary` counters. The mandatory advisory-only paragraph from the capability specification "Advisory-only framing" section appears at the bottom of the file, verbatim.
