...
Load it from `workflow:paper.check` (the citation block), from `workflow:paper.lit` (after a fresh import), and from the `evidence-auditor` subagent (when it walks the matrix).
