...
description: >
  ...
  searching code by symbol, or running `workflow:paper.code-audit`,
  `workflow:patent.scout`, or `workflow:patent.disclosure`. Sister skill
...
references:
  ...
  - doc: workflow:paper.code-audit
  - doc: workflow:patent.scout
---
...
- `workflow:paper.code-audit` — refresh `repo_summary.md` and `method_to_code.md` rows; gather `file:line` citations for the audit table.
- `workflow:patent.scout` — find non-obvious technical mechanisms and back each candidate with `file:line` evidence.
- `workflow:patent.disclosure` "Code traceability" section — reuse `method_to_code.md`.
...
`policy:workspace-safety` preflight blocks reads of `.env*`, `secrets/`, `**/*.pem`, `**/*.key`, and `credentials.json`. Never override it; if a config references a secret path, record only the path.
...
- [ ] No file matched by `policy:workspace-safety` was opened.
...
- Reading `.env` / `secrets/` / `*.pem` / `*.key` — `policy:workspace-safety` blocks; do not attempt to override.
