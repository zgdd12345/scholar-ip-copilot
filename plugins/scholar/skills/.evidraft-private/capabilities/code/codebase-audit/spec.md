...
phase: shared
description: >
  Load when workflow:paper.code-audit or workflow:patent.scout runs, or whenever a claim about
...
- `workflow:paper.code-audit` builds `repo_summary.md`, `method_to_code.md`, and `paper_code_audit.md`.
- `workflow:patent.scout` needs code evidence for each candidate.
- `workflow:patent.disclosure` "Code traceability" section reuses `method_to_code.md`.
...
- [ ] `policy:workspace-safety` paths (`.env`, `secrets/`, `*.pem`, `*.key`) never opened.
