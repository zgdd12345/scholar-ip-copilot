...
| `refine` | The contribution is real but at least one of: prior-work contrast is missing, evaluation plan is hand-waved, claim type is undecided, disclosure status is unclear. Another `workflow:scope.run` round is needed before downstream work. |
