...
This rule applies to the Tier 1 provider JSON cache only. The Tier 2 snapshot store at `.evidraft/literature/snapshots/` (used by the `webfetch` variant) is durable evidence backing — never delete those files, even if older than 14 days; see `capability specification` §Tier 2.
