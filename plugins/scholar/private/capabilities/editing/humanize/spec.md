...
phase: paper
description: >
  Loaded by workflow:polish.run. Ships ~12 concrete rewriting rules (Williams /
...
Loaded whenever `workflow:polish.run` runs, or whenever an agent considers a "make this read more naturally" pass on a draft. The skill is the single source of truth for the rules, the banned-phrase list, the ethics statement, the model-routing config, and the diff-log shape. The command and the `prose-polisher` agent both defer to it.
...
| Ethics + refusal | [ethics-and-refusal.md](references/ethics-and-refusal.md) | verbatim ethics block; full refusal-flag list + regex; venue policies (ICML / NeurIPS / ACL / IEEE) — `workflow:polish.run` anchors here |
