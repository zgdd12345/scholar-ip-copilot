...
Never run two providers' calls in parallel within a single `workflow:research.deep` Stage 2 sub-query; cross-provider parallelism is the orchestrator's job, this skill is per-call.
