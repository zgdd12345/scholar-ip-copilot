...
phase: paper
description: >
  Load when workflow:paper.draft, workflow:paper.venue, or workflow:paper.check run. Provides the
...
references:
  ...
  - doc: capability:venue-formatting
...
Load whenever LaTeX source under `manuscript/` is being created, edited, or compiled. The skill is also the reference for `workflow:paper.venue` (which copies a manuscript and rewrites the preamble) and `workflow:paper.check` (which audits compile output).

Compile and error parsing are owned by `capability:latex-build`, which wraps `latexmk` and parses `main.log` into a structured taxonomy. Invoke that capability instead of calling `latexmk` ad hoc.
...
The default style is arXiv-neutral: `\documentclass{article}`, single column, no venue assumptions. Keep `manuscript/main.tex` style-stable so `workflow:paper.venue` can swap the preamble without touching prose.
...
- The preamble is the **only** thing `workflow:paper.venue` should rewrite. Section prose should not break under a different preamble.
...
- One label per object. Two figures sharing a label is silent breakage; `workflow:paper.check` flags it.
...
- Don't hard-code page limits in the prose (no "in this 8-page paper"). Length is a `workflow:paper.venue` concern.
...
| `LaTeX Warning: Label multiply defined.` | duplicate label | two captions share a label | rename one — `workflow:paper.check` will list both |
...
4. After `0 errors, 0 warnings (or only overfull \hbox warnings)`, move on. Stop trying to silence overfull boxes unless `workflow:paper.check` flags them at submission.
