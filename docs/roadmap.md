...
- [x] Adapter conformance test suite — `tests/test_adapter_conformance.py` (18 parametrised tests across 9 invariants: A file-count, B frontmatter-survival, C retention preamble, D MCP-reference ratchet, E required-skill (existence + rendering), F subagent-dispatch, G executable hooks, H per-agent model + effort, I skill-bundle propagation; across claude-code / codex-cli / opencode). Full `pytest tests/` reports 46 passing.
