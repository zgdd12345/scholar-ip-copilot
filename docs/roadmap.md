...
- [x] Adapter conformance test suite — `tests/test_adapter_conformance.py` (22 parametrised tests across 12 invariants: A file-count, B frontmatter-survival, C retention preamble, D MCP-reference ratchet, E required-skill (existence + rendering), F subagent-dispatch, G executable hooks, H per-agent model + effort, I skill-bundle propagation, J relative-link integrity (source markdown), K relative-link integrity (rendered output, catches adapter path-flattening bugs), L frontmatter `references[].doc:` integrity; across claude-code / codex-cli / opencode). Full `pytest tests/` reports 53 passing.
