...
Outcome metric (persistent context drop across the nine splits):

| Source files refactored | Before | After | Reduction |
|---|---:|---:|---:|
| `commands/deepresearch.md` + 8 SKILL.md files | 2533 | 1025 | **-60%** |

Plus 44+ on-demand `references/*.md` files that ship to every host via bundle propagation.
...
| 9 | `skills/latex-style-audit/SKILL.md` | 263 | **93** | -65% | _this commit_ | 4 (rule-taxonomy + output-schemas + procedure + anti-patterns) |
| | **TOTAL** | **2533** | **1025** | **-60%** | | **44** new references files (plus 6 for `using-deep-research` — `eefef20`) |
...
Rule of thumb confirmed by the nine done splits: **≥ 263 lines is split-worth (latex-style-audit 263→93 is the new lower bound); 230-260 is judgement call; ≤ 220 is fine as a single file** under Anthropic's recommended 300-line bound.
